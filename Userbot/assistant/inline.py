...
#
from Userbot.helper.tools._button import pingx_button
#
__MODULES__ = "Inline"
...
async def pingx_inline(jawab, iq, _):
    from Userbot import nlx
    import psutil, requests
    from datetime import datetime
    from time import time
    from Userbot.helper.tools import Emojik, get_time, initial_ctext, start_time
    from pyrogram.raw.functions import Ping
    c = next((x for x in nlx._ubot if x.me.id == iq.from_user.id), nlx._ubot[0])
    em = Emojik(c)
    em.initialize()
    start = datetime.now()
    ...
    country = "-"
    try:
        req = requests.get("https://ipinfo.io/country", timeout=5)
        if req.ok:
            country = req.text.strip()
    except Exception:
        pass
    cpu = f"{psutil.cpu_percent()}%"
    mem = f"{psutil.virtual_memory().percent}%"
    userbot_name = f"{c.me.first_name} {c.me.last_name or ''}"
    details = (
        f"<b>🖥️ Userbot Status</b>\n"
        f"<b>• Uptime:</b> <code>{uptime}</code>\n"
        f"<b>• Negara Server:</b> <code>{country}</code>\n"
        f"<b>• CPU:</b> <code>{cpu}</code>\n"
        f"<b>• Memori:</b> <code>{mem}</code>\n"
        f"<b>• Userbot:</b> <code>{userbot_name}</code>"
    )
    # details = "<b>Userbot Ping</b>\n<b>Status:</b> OK"
    ...
    jawab.append(
        InlineQueryResultArticle(
            ...
            reply_markup=pingx_button(user_id),
            description="Status Userbot + Button Ping Now"
        )
    )
    return jawab
...
async def _(c, q, _):
    try:
        ...
        if text.split()[0] == "pingx":
            answerss = await pingx_inline(answers, q, _)
...
async def inline_help(j, iq, _):
    ...
    msg = "**<blockquote><b>🎛️ Menu@Lite_Xbott : `{}`</b></blockquote>\n<blockquote><b><i>     🪬 Prefixes: `{}`\n     📇 User: `{}`**</b></i></blockquote>".format(
        plugin, " ".join(prefix), full,
    )
...
<b>{nama_bot}</b>
    <b>status:</b> {stutas} 
      ...
      <b>prox_uptime:</b> <code>{upnya}</code>
