...
async def temp_mail(client, message, *args):
    ...
    url = f"https://api.botcahx.eu.org/api/tools/create-temp-mail?apikey={botcax_api}"
    data = fetch_json(url)
...
async def check_mail(client, message, *args):
    ...
    url = f"https://api.botcahx.eu.org/api/tools/cek-msg-tmp-mail?email={email}&apikey={botcax_api}"
    data = fetch_json(url)
