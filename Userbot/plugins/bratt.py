...
#@moire_mor
# === FONT MAP OTOMATIS dari Userbot/font ===
...
def generate_font_map():
    ...
    for fname in os.listdir(FONT_DIR):
        if fname.lower().endswith(".ttf"):
            # Kode unik: 2 huruf awal tiap kata, maksimal 8 karakter
...
DEFAULT_SETTINGS = {
    "anim": "none",
    "font": next(iter(FONT_MAP.keys()), ""),  # kode font pertama sebagai default
    "warna": "black",
    "style": "none"
}
...
def show_user_settings(user_id):
    ...
    text = (
        ...
        f"\nGunakan <code>.brattvidset help</code> untuk bantuan."
    )
    return text
#@moire_mor
def get_font_help():
...
<b>Cara Pakai:</b>
<code>.brattvidset [opsi] [value]</code>
...
<b>Contoh:</b>
<code>.brattvidset anim fade</code>
<code>.brattvidset font anbo</code>
<code>.brattvidset warna gold</code>
<code>.brattvidset style outline</code>
<code>.brattvidset show</code>
<code>.brattvidset reset</code>
...
#@moire_mor
        ...
        await message.reply_text("Opsi tidak dikenali! Gunakan <code>.brattvidset help</code>.")
...
def get_font(fontkey):
    ...
    if not fontinfo:
        raise Exception(f"Font '{fontkey}' tidak ditemukan di daftar FONT_MAP. Cek kode font di .brattvidset help")
...
async def brat_handler(client, message: Message, *args):
    ...
    # Cek background: reply ke gambar/sticker
